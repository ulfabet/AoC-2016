...
func valid_move(x1,y1,x2,y2 int) bool {
    ...
    s := nodes[y1][x1]
    d := nodes[y2][x2]
    if s.goal && x2 > x1 { return false } // don't move goal right?
...
func search_neighbours(x, y int) {
    ...
    for _,c := range neighbours_coords(x, y) {
        //fmt.Println(c)
...
func fifo_add(x,y,x2,y2 int) {
    fifo = append(fifo, []int{x,y,x2,y2})
}
