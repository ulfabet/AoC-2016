...
func walk(x, y, n int) {
    ...
    for i,pc := range poi {
        if pc == nil { continue }
        if x == pc[0] && y == pc[1] {
            fmt.Println("Reached POI", i, "in", n-1, "steps")
            dist[si][i] = n-1
        }
    }
...
func main() {
    ...
    for si=0; si<len(list); si++ {
        fmt.Println("Start from", si)
        clear()
        fifo = [][]int{{poi[si][0], poi[si][1], 1}}
        fi = 0
        total = 0 // not used
        for fi < len(fifo) {
            //show()
